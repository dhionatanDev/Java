...
public class Main {
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= testes ; i ++) {
			System.out.println("Quantas cobaias foram usadas no teste nº:" + i);
			quantia = sc.nextFloat();
			
			while (quantia > 0 && quantia < 16) {
				
				System.out.println("Qual é o tipo de cobaia R = ratos, S = Sapos, C = Coelhos ");
				...
				if (tipo == 'R') {
					...
					if (tipo == 'C') {
						...
						qntS = sc.nextFloat();
							...
							System.out.println("Erro, você informou o tipo errado:");
						}
					}
				}
				quantia = quantia - qntR - qntC - qntS;
				if (quantia != 0) {
				System.out.println("faltam informar: " + quantia + " Cobaias " + " no " + i + "º teste");
